# -*- coding: utf-8 -*-
"""
Alternative-Objektiv-Auswertung (Tweedie/Gamma/Hazard/Quantil inkl. P80) auf der
PROSPEKTIVEN Kohorte OHNE is_open-Filter (n=286 = 193 abgeschlossen + 93 offen/zensiert).
Identische rekonstruierte 24h-Features wie das finale ExtraTrees, nur andere Zielfunktionen.

Liest:
  alt_matrices/retro_train.parquet            (Training; aus prospective_24h_rebuild.py)
  alt_matrices/feature_lists.json             (present/numc/cat)
  alt_matrices_no_isopen/prospective_rebuilt_286.parquet  (aus 08_no_isopen_sensitivity.py)

ACHTUNG: bei is_open=1 ist __los__ ZENSIERT (bisher vergangene Zeit, nicht endgueltige LoS).
Fehler dieser Stays sind gegen eine Untergrenze bewertet -> nur Sensitivitaet/Kontrast.
"""
...
OUT=OUTX/"los_alt_prospective_286"
...
pp=pd.read_parquet(ALT286/"prospective_rebuilt_286.parquet")
...
print(f"Retro-Train {Xtr_df.shape} | prospektiv rekonstruiert {Xp_df.shape} (n={len(los)}, "
      f"abgeschlossen {int((isopen==0).sum())}, offen/zensiert {int((isopen==1).sum())})")
...
Xp=Xp[COLS]
Xtr_v=Xtr.to_numpy(dtype=np.float64,na_value=np.nan); Xp_v=Xp.to_numpy(dtype=np.float64,na_value=np.nan)
...
preds["Oberarzt"]=arzt
# kanonisches ExtraTrees (finales Modell) fuer alle 286 — aus 08_no_isopen_sensitivity.py vorberechnet
if "__pred_ExtraTrees__" in pp.columns:
    preds["ExtraTrees"]=pp["__pred_ExtraTrees__"].to_numpy(float)
else:
    print("ExtraTrees-Vorhersagen fehlen in der Matrix — 08_no_isopen_sensitivity.py neu ausfuehren.")
...
# Gesamt zusätzlich
for o in order: rows.append({**met(los,preds[o],o),"Subgroup":"overall"})
res=pd.DataFrame(rows)[["Subgroup","Modell","n","MAE","MedianAE","RMSE","R2","Bias"]]
res.to_csv(f"{OUT}.csv",sep=";",index=False)
print("\n=== PROSPEKTIV OHNE is_open-Filter, rekonstruierte 286-Features (Tage) ===")
...
ax.set_title("KISIK prospective WITHOUT is_open filter (n=286, reconstructed features): physician vs ML approaches by actual ICU LoS\n"
             "[is_open=1 LoS = elapsed time only — MAE for open stays scored against a lower bound]",weight="bold",fontsize=10.5)
