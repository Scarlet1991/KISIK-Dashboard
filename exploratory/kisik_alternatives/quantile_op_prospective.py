...
allowed = [("AIN","IZ32"),("AIN","IZ21"),("AIN","IZ31")]  # nur AIN-Intensiveinheiten (apples-to-apples mit MIMIC + Manuskript)
...
dp = con.execute(f"SELECT * FROM read_parquet('{PROS.as_posix()}') WHERE (wardshort,oebenekurz) IN ({allowed_sql}) AND is_open=0 AND icu_duration_h/24.0 > 1").df()  # kanonische 193er-Kohorte (AIN, abgeschlossen, LOS>1)
